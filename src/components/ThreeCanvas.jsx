...
function ThreeCanvas({ 
  ...
  useEffect(() => {
    ...
    // Helper for subtle ambient dust
    const createAmbientDust = (count = 350, size = 0.025, color = 0x0284c7) => {
      ...
      const mat = new THREE.PointsMaterial({ size, color, transparent: true, opacity: 0.4 });
      const points = new THREE.Points(geo, mat);
      scene.add(points);
      return points;
    };

    // --- 25 CREATIVE PRESET RENDERERS ---
    switch (currentPreset) {
      case 'mobiusStrip': {
        const geo = new THREE.TorusGeometry(2, 0.3, 16, 100);
        const mat = new THREE.MeshBasicMaterial({ color: 0x0284c7, wireframe: true, transparent: true, opacity: 0.16 });
        const mobius = new THREE.Mesh(geo, mat);
        mobius.position.set(2.5, 0, -1);
        mobius.rotation.x = Math.PI / 3;
        scene.add(mobius);
        const dust = createAmbientDust(300, 0.025, 0x0f766e);
      ...
      case 'dodecahedronCluster': {
        ...
        const mat = new THREE.MeshBasicMaterial({ color: 0x0f766e, wireframe: true, transparent: true, opacity: 0.18 });
        const dodeca = new THREE.Mesh(geo, mat);
        dodeca.position.set(2.5, 0, -1);
        scene.add(dodeca);
        const dust = createAmbientDust(350, 0.025, 0x38bdf8);
        animatedObjects.push({
          update: (time) => {
      ...
      case 'blackHoleVortex': {
        ...
        for (let i = 0; i < count; i++) {
          ...
          pos[i * 3] = r * Math.cos(theta) + 2.5;
          pos[i * 3 + 1] = (Math.random() - 0.5) * 0.8;
          pos[i * 3 + 2] = r * Math.sin(theta) - 1;
        }
        geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        const mat = new THREE.PointsMaterial({ size: 0.035, color: 0x0284c7, transparent: true, opacity: 0.5 });
      ...
      case 'neuralNetworkLattice': {
        ...
        for (let i = 0; i < nodeCount; i++) {
          ...
          const pMat = new THREE.MeshBasicMaterial({ color: i % 2 === 0 ? 0x0284c7 : 0x0f766e, transparent: true, opacity: 0.6 });
          const p = new THREE.Mesh(pGeo, pMat);
          p.position.set((Math.random() - 0.5) * 8, (Math.random() - 0.5) * 6, (Math.random() - 0.5) * 5);
          group.add(p);
          nodes.push(p);
        }
        for (let i = 0; i < nodeCount; i++) {
          for (let j = i + 1; j < nodeCount; j++) {
            if (nodes[i].position.distanceTo(nodes[j].position) < 3.2) {
              const lineMat = new THREE.LineBasicMaterial({ color: 0x0284c7, transparent: true, opacity: 0.15 });
      ...
      case 'holographicPyramid': {
        ...
        for (let i = 0; i < 4; i++) {
          const geo = new THREE.ConeGeometry(1.2 + i * 0.4, 1.8 + i * 0.3, 4);
          const mat = new THREE.MeshBasicMaterial({ color: 0x0284c7, wireframe: true, transparent: true, opacity: 0.15 });
      ...
      case 'particleTunnel': {
        ...
        for (let i = 0; i < count; i++) {
          ...
          pos[i * 3] = radius * Math.cos(theta) + 2.5;
          pos[i * 3 + 1] = radius * Math.sin(theta);
          pos[i * 3 + 2] = (Math.random() - 0.5) * 20;
        }
        geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        const mat = new THREE.PointsMaterial({ size: 0.04, color: 0x0284c7, transparent: true, opacity: 0.5 });
      ...
      case 'atomModel': {
        ...
        const coreMat = new THREE.MeshBasicMaterial({ color: 0x0284c7, transparent: true, opacity: 0.5 });
        group.add(new THREE.Mesh(coreGeo, coreMat));

        for (let i = 0; i < 3; i++) {
          const ringGeo = new THREE.TorusGeometry(2 + i * 0.3, 0.02, 16, 80);
          const ringMat = new THREE.MeshBasicMaterial({ color: 0x0f766e, transparent: true, opacity: 0.3 });
      ...
      case 'cyberHexagonGrid': {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          const mat = new THREE.MeshBasicMaterial({ color: 0x0284c7, wireframe: true, transparent: true, opacity: 0.18 });
      ...
      case 'auroraWaveBands': {
        ...
        for (let l = 0; l < lineCount; l++) {
          ...
          for (let x = -10; x <= 10; x += 0.5) {
            points.push(new THREE.Vector3(x, Math.sin(x * 0.4 + l) * 1.2, (l - lineCount / 2) * 0.5));
          }
          const lineGeo = new THREE.BufferGeometry().setFromPoints(points);
          const lineMat = new THREE.LineBasicMaterial({ color: l % 2 === 0 ? 0x0284c7 : 0x0f766e, transparent: true, opacity: 0.2 });
          group.add(new THREE.Line(lineGeo, lineMat));
        }
        scene.add(group);
        animatedObjects.push({
      ...
      case 'stellarConstellationGlobe': {
        ...
        const mat = new THREE.MeshBasicMaterial({ color: 0x0284c7, wireframe: true, transparent: true, opacity: 0.12 });
      ...
      // Existing 15 Presets (Tuned for Soft Opacity & Readability)
      case 'cyberGrid': {
        const gridHelper = new THREE.GridHelper(50, 40, 0x0284c7, 0x1e293b);
        gridHelper.position.y = -3;
        gridHelper.rotation.x = 0.15;
        scene.add(gridHelper);
        const dust = createAmbientDust(400, 0.03, 0x0f766e);
        animatedObjects.push({
          update: (time) => {
      ...
      case 'dnaHelix': {
        ...
        for (let i = 0; i < strandCount; i++) {
          ...
          const p1Geo = new THREE.SphereGeometry(0.06, 6, 6);
          const p1Mat = new THREE.MeshBasicMaterial({ color: 0x0284c7, transparent: true, opacity: 0.5 });
          const p1 = new THREE.Mesh(p1Geo, p1Mat);
          p1.position.set(x1, y, z1);
          dnaGroup.add(p1);
          const p2Geo = new THREE.SphereGeometry(0.06, 6, 6);
          const p2Mat = new THREE.MeshBasicMaterial({ color: 0x0f766e, transparent: true, opacity: 0.5 });
        ...
        const dust = createAmbientDust(300, 0.025, 0x0284c7);
        scene.add(dnaGroup);
        animatedObjects.push({
          update: (time) => {
      ...
      case 'torusKnot': {
        const geo = new THREE.TorusKnotGeometry(2, 0.45, 100, 16);
        const mat = new THREE.MeshBasicMaterial({ color: 0x0284c7, wireframe: true, transparent: true, opacity: 0.14 });
        const mesh = new THREE.Mesh(geo, mat);
        mesh.position.set(2.5, 0, -1.5);
        scene.add(mesh);
        const dust = createAmbientDust(350, 0.025, 0x0f766e);
        animatedObjects.push({
          update: (time) => {
      ...
      case 'cosmicConstellation': {
        ...
        geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        const mat = new THREE.PointsMaterial({ size: 0.04, color: 0x38bdf8, transparent: true, opacity: 0.5 });
        const points = new THREE.Points(geo, mat);
        scene.add(points);
        animatedObjects.push({
          update: () => {
      ...
      case 'quantumWaves': {
        ...
        geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        const mat = new THREE.PointsMaterial({ size: 0.035, color: 0x0284c7, transparent: true, opacity: 0.45 });
      ...
      case 'matrixRain': {
        ...
        geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        const mat = new THREE.PointsMaterial({ size: 0.04, color: 0x059669, transparent: true, opacity: 0.5 });
      ...
      case 'hyperCube': {
        ...
        const outerMat = new THREE.MeshBasicMaterial({ color: 0x0284c7, wireframe: true, transparent: true, opacity: 0.15 });
        const outerBox = new THREE.Mesh(outerGeo, outerMat);
        const innerGeo = new THREE.BoxGeometry(1.5, 1.5, 1.5);
        const innerMat = new THREE.MeshBasicMaterial({ color: 0x0f766e, wireframe: true, transparent: true, opacity: 0.25 });
        const innerBox = new THREE.Mesh(innerGeo, innerMat);
        const cubeGroup = new THREE.Group();
        cubeGroup.position.set(2.5, 0, -1);
        cubeGroup.add(outerBox);
        cubeGroup.add(innerBox);
        const dust = createAmbientDust(300, 0.025, 0x38bdf8);
        scene.add(cubeGroup);
        animatedObjects.push({
          update: (time) => {
      ...
      case 'neonPlexus': {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          const mat = new THREE.MeshBasicMaterial({ color: i % 2 === 0 ? 0x0284c7 : 0x0f766e, wireframe: true, transparent: true, opacity: 0.16 });
          const mesh = new THREE.Mesh(geo, mat);
      ...
      case 'gyroscopeRings': {
        ...
        const ring1Mat = new THREE.MeshBasicMaterial({ color: 0x0284c7, transparent: true, opacity: 0.18 });
        const ring1 = new THREE.Mesh(ring1Geo, ring1Mat);
        const ring2Geo = new THREE.TorusGeometry(2.2, 0.03, 16, 100);
        const ring2Mat = new THREE.MeshBasicMaterial({ color: 0x0f766e, transparent: true, opacity: 0.22 });
        const ring2 = new THREE.Mesh(ring2Geo, ring2Mat);
        const gyroGroup = new THREE.Group();
        gyroGroup.position.set(2.5, 0, -1.5);
        gyroGroup.add(ring1);
        gyroGroup.add(ring2);
        scene.add(gyroGroup);
        const dust = createAmbientDust(300, 0.025, 0x38bdf8);
      ...
      case 'particleCore': {
        ...
        const coreMat = new THREE.MeshBasicMaterial({ color: 0x0284c7, wireframe: true, transparent: true, opacity: 0.14 });
        ...
        const pMat = new THREE.PointsMaterial({ size: 0.035, color: 0x0f766e, transparent: true, opacity: 0.45 });
      ...
      case 'warpSpeed': {
        ...
        geo.setAttribute('position', new THREE.BufferAttribute(pos, 3));
        const mat = new THREE.PointsMaterial({ size: 0.045, color: 0x0284c7, transparent: true, opacity: 0.5 });
      ...
      case 'circuitGrid': {
        ...
        const linesMat = new THREE.LineBasicMaterial({ color: 0x0284c7, transparent: true, opacity: 0.18 });
        for (let i = 0; i < 20; i++) {
          const points = [];
      ...
      case 'crystalOctahedrons': {
        ...
        for (let i = 0; i < 10; i++) {
          const geo = new THREE.OctahedronGeometry(0.6 + Math.random() * 0.4, 0);
          const mat = new THREE.MeshBasicMaterial({ color: i % 2 === 0 ? 0x0284c7 : 0x0f766e, wireframe: true, transparent: true, opacity: 0.16 });
          const mesh = new THREE.Mesh(geo, mat);
      ...
      // Default: PRESET #1 GLOWING TECH SPHERE (SOFT AMBIENT OPACITY)
      default:
      case 'icosahedron': {
        ...
        const particlesMaterial = new THREE.PointsMaterial({ size: 0.03, color: 0x0284c7, transparent: true, opacity: 0.45 });
        const particlesMesh = new THREE.Points(particlesGeometry, particlesMaterial);
        scene.add(particlesMesh);

        const geometry = new THREE.IcosahedronGeometry(2.2, 1);
        const material = new THREE.MeshBasicMaterial({ color: 0x0f766e, wireframe: true, transparent: true, opacity: 0.15 });
